...
async function retry(action, predicateOrRetryCount = 4, cooldown = 250) {
	...
	if (typeof predicateOrRetryCount === "number") {
		let tries = 0;
		if (predicateOrRetryCount > maxTries) {
			throw new Error("Retry count is larger than limit: " + maxTries);
		}
...
function generate_unsafe_id(length) {
	if (!length) {
...
function style_string(style_object) {
	const camelToKebab = (input) => input.replace(/[A-Z]+(?![a-z])|[A-Z]/g, ($, ofs) => (ofs ? "-" : "") + $.toLowerCase());
	const res = Object.keys(style_object).map(property => `${camelToKebab(property)}: ${style_object[property]};`).join("\n");
	return res;
}
...
function create_element(name, properties, children) {
	...
	for (const [key, value] of Object.entries(properties)) {
		...
		if (key === "style") {
			node.style.cssText = style_string(value);
			continue;
		}
		node[key] = value;
	}
